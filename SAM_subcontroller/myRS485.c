...
void RS485_1_Init(){
    ...
    UARTConfigSetExpClk(UART1_BASE, SysCtlClockGet(), 1500000,
                        (UART_CONFIG_WLEN_8 | UART_CONFIG_STOP_ONE | UART_CONFIG_PAR_NONE));
    ...
    UARTFIFOLevelSet(UART1_BASE,UART_FIFO_TX1_8,UART_FIFO_RX2_8);
    UARTTxIntModeSet(UART1_BASE,UART_TXINT_MODE_EOT);
    UARTIntEnable(UART1_BASE,  UART_INT_RT|UART_INT_RX|UART_INT_TX);
...
void UART1_Interrupt_Handler(void)
{
    ...
    else if(interrupt_status&(UART_INT_RT|UART_INT_RX))
    {
        ...
        {
            charData[dataCount]=(char)UARTCharGet(UART1_BASE);
            dataCount++;
        }
        if(dataCount==2)
        {
            if(samReadBusy)
            {
                switch(samReadMode){
                case SAMREAD_POS8_:
                    samLoad8[samReadCurrentID_C1]=(unsigned char)charData[0];
                    samPosition8[samReadCurrentID_C1]=(unsigned char)charData[1];
                    break;
                case SAMREAD_POS12_:
                    samPosition12[samReadCurrentID_C1]=((charData[0]&0x1F)<<7)+(charData[1]&0x7F);
                    break;
                case SAMREAD_PD_:
                    samP[samReadCurrentID_C1]=charData[0];
                    samD[samReadCurrentID_C1]=charData[1];
                    break;
                case SAMREAD_I_:
